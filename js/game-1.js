...
const firstGameScreen = renderElement(firstGameTemplate);
const formElement = firstGameScreen.querySelector(`.game__content`);
const questionsQuantity = formElement.querySelectorAll(`.game__option`).length;

const formElementChangeHandler = () => {
  const radioInputs = Array.from(formElement.querySelectorAll(`input[type="radio"]`));
  const answers = radioInputs.filter((input) => input.checked);
  if (answers.length === questionsQuantity) {
